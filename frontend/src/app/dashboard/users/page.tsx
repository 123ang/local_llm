  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-slate-900">Users</h1><p className="text-slate-500 mt-1">Assign users to the correct departments</p></div>
        {isSuperAdmin && <button onClick={openCreateForm} className="flex items-center gap-2 px-4 py-2.5 rounded-lg bg-red-600 hover:bg-red-700 text-white text-sm font-medium"><Plus size={16} /> Add User</button>}
      </div>
      ...
      {showForm && isSuperAdmin && (
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><label className="block text-sm font-medium text-slate-700 mb-1">Role</label><select value={form.role} onChange={e => setForm({...form, role: e.target.value, department_ids: e.target.value === "super_admin" ? [] : form.department_ids})} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm"><option value="user">User</option><option value="admin">Organization Admin</option><option value="super_admin">Super Admin</option></select></div>
            <div><label className="block text-sm font-medium text-slate-700 mb-1">Organization</label><select value={form.company_id} onChange={e => setForm({...form, company_id: e.target.value, department_ids: []})} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm"><option value="">No organization (platform)</option>{companies.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
            <div className="col-span-2">
              <label className="block text-sm font-medium text-slate-700 mb-2">Initial Department Access</label>
              <div className="flex flex-wrap gap-2">
                {departments.map((department) => (
                  ...
                  >
                    {department.id === generalDepartmentId ? "General (default)" : department.name}
                  </button>
                ))}
                {form.company_id && departments.length === 0 && <span className="text-xs text-slate-400">No departments yet</span>}
              </div>
            </div>
          </div>
      ...
      {grantUser && (
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-lg font-semibold">Department Access</h2>
          ...
          <div className="mt-4 flex gap-2">
            <button onClick={saveGrants} disabled={loading} className="px-4 py-2 rounded-lg bg-red-600 hover:bg-red-700 text-white text-sm font-medium disabled:opacity-50">Save Access</button>
            <button onClick={() => setGrantUser(null)} className="px-4 py-2 rounded-lg border border-slate-300 text-sm text-slate-600 hover:bg-slate-50">Cancel</button>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <table className="w-full">
          <thead className="bg-slate-50 border-b border-slate-200"><tr>
            ...
            <th className="text-left px-6 py-3 text-xs font-semibold text-slate-500 uppercase">Departments</th>
              ...
              <tr key={u.id} className="hover:bg-slate-50">
                ...
                <td className="px-6 py-4">
                  <div className="flex flex-wrap gap-1">
                    ...
                    {(u.departments || []).length === 0 && <span className="text-sm text-slate-400">No access</span>}
                  </div>
                </td>
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center justify-end gap-2">
                    <button onClick={() => openGrantPanel(u)} className="text-slate-400 hover:text-red-600" title="Manage department access"><KeyRound size={18} /></button>
